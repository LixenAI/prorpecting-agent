import { getQualificationStatus, scoreLead } from "./scoring";
import type {
  Lead,
...
function leadFromInput(input: ProspectRouteInput): Lead {
  const now = new Date().toISOString();
  return {
    id: "route-eval",
    businessName: trimOrUndefined(input.businessName) ?? "",
    ...
    facebook: trimOrUndefined(input.facebook),
    source: trimOrUndefined(input.source) ?? "ghl_agent_studio",
...
export function routeProspect(input: ProspectRouteInput): ProspectRouteDecision {
  ...
  if (leadScore == null && hasScoringSignal) {
    const breakdown = scoreLead(leadFromInput(input));
